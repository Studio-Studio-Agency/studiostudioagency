...
Statische Website fuer die **Haito GmbH**, Basel-Stadt. Lokale KI-Systeme
fuer Schweizer KMU, Beratung zu Datensouveraenitaet.

```sh
npm install
npm run dev        # Entwicklungsserver auf http://localhost:4321/haito
npm run build      # Vorschau-Build: dev.studiostudio.ch/haito
npm run build:live # Live-Build: haito.ch
npm run preview    # Ausgabe lokal pruefen
```

Die Seite laeuft zuerst unter **`dev.studiostudio.ch/haito`**, also in
...
|---|---|---|
| `SITE_URL` | `https://dev.studiostudio.ch` | `https://haito.ch` |
| `BASE_PATH` | `/haito` | `/` |
