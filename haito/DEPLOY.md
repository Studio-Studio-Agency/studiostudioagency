...
|---|---|---|---|
| Adresse | `dev.studiostudio.ch/haito` | `haito.studiostudio.ch` | `haito.ch` |
| `SITE_URL` | `https://dev.studiostudio.ch` | `https://haito.studiostudio.ch` | `https://haito.ch` |
| `BASE_PATH` | `/haito` | `/` | `/` |
...
Brief zielt auf `haito.ch`. Kaeme `haito.studiostudio.ch` zuerst in den
...
```sh
npm ci
npm run build        # Vorschau: dev.studiostudio.ch/haito
npm run build:live   # Live: haito.ch
```
...
```sh
npm run preview   # http://localhost:4321/haito
```
...
**Vorschau** — in das Unterverzeichnis `haito/` des Web-Wurzelverzeichnisses
von `dev.studiostudio.ch`:

```sh
npm run build
rsync -avz --delete dist/ BENUTZER@HOST:PFAD_ZUR_DOMAIN/haito/
```
...
betrifft das nur `haito/`, weil rsync nur dieses Verzeichnis anfasst.
Alternativ ueber den Dateimanager oder FTP.

### Automatisch hochladen (empfohlen)

`.github/workflows/deploy-haito.yml` laedt per GitHub Actions hoch. Bewusst
nur von Hand ausloesbar (Actions → «Haito hochladen» → Run workflow →
...
|---|---|---|---|
| Variable | `SITE_URL` | `https://dev.studiostudio.ch` | `https://haito.ch` |
| Variable | `BASE_PATH` | `/haito` | `/` |
...
(`ssh-keygen -t ed25519 -C "github-actions-haito"`), den oeffentlichen Teil
...
`dev.studiostudio.ch/haito` steht auf einer fremden, oeffentlich
...
`dev.studiostudio.ch/haito/robots.txt`. Die vom Build erzeugte Datei
...
- **404:** `dist/404.html` als Fehlerseite eintragen. In der Vorschau
  `ErrorDocument 404 /haito/404.html`, live `ErrorDocument 404 /404.html`.
...
- [ ] Vorschau unter `dev.studiostudio.ch/haito` abgeschaltet oder
