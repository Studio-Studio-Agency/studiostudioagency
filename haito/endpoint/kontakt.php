<?php
/**
 * Kontaktformular-Endpunkt fuer haito.ch
 *
...
function abbrechen(string $meldung, int $status = 400): never {
    ...
    <title>Anfrage nicht gesendet | Haito</title>
    <style>
...
$spur = sys_get_temp_dir() . '/haito-kontakt-' . sha1($ip . $config['sperre_salz']);
$jetzt = time();
...
$betreff = sprintf('Anfrage über haito.ch — %s, %s', $name, $firma);
...
if (!$ok) {
    error_log('haito: mail() fehlgeschlagen fuer ' . $email);
